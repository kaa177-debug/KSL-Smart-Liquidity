...
Entry methods are breakout close, breakout FVG, and the default retest. A bullish retest trades below/touches the prior range high and closes back above it; the bearish rule is symmetrical. Range height is retained for scoring and measured-move context. The dashboard shows only the nearest directional liquidity target and structural Stop Loss.
